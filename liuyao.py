...
class liuyao:
    def __init__(self, root, gua64):
        ...
        self.tfont = tkinter.font.Font(family='Fixdsys', size=25, weight=tkinter.font.BOLD)
        self.gfont = tkinter.font.Font(family='Fixdsys', size=13, weight=tkinter.font.BOLD)
        self.dgua = []  # 动爻
        self.ygua = []  # 原始卦
        self.bbgua = []  # 变卦
        self.yguastr = []  # 原始卦
    ...
    def liushenf(self):  # 配六神
        xu = self.liushen[self.ritian]
        for i in range(6):
            self.canvas.create_text(self.guax - 170, 100 + 150 - 30 * i, font=self.gfont, text=self.liushencn[xu],
                                    tag='liushen')
            xu += 1
            if xu > 5:
                xu -= 6
    ...
    def date(self):
        ...
        #url = 'https://www.nongli.com/plus/riliajax.php', params={'Year': 'today.year', 'Month': 'today.month', 'Day': 'today.day'}
        print(today.year)
        resp = requests.get('https://www.nongli.com/plus/riliajax.php', params={'Year': today.year, 'Month': today.month, 'Day': today.day}).json()
        zu = resp['dizhi']
        wanzu=zu.split(u'\u3000') #wanzu里面是完成的年月日干支
    ...
    def kongwang(self):
        ...
        self.canvas.create_text(self.guax - 90 + 120 + 30 + 15, 30, font=self.gfont, text=self.kongwangzu[cha - 2],
                                tag='riqi')
        self.canvas.create_text(self.guax - 90 + 120 + 30 + 30, 30, font=self.gfont, text=self.kongwangzu[cha - 1],
                                tag='riqi')
    ...
    def draw(self):
        ...
        for i in range(6):
            if self.gua[i] == '1':
                ...
                self.canvas.create_text(self.guax, 95 + 150 - 30 * i, text='▅▅▅', font=('Fixdsys', 17, tkinter.font.BOLD),
                                        tag='pic')
            else:
        ...
        for i in range(0, len(self.dgua), 2):
            if self.dgua[i + 1] == '1':
                self.canvas.create_text(self.guax + 70, 247 - 30 * int(self.dgua[i]), font=self.gfont, text='O', tag='pic',
                                        fill='red')
            else:
                self.canvas.create_text(self.guax + 70, 247 - 30 * int(self.dgua[i]), font=self.gfont, text='X', tag='pic',
                                        fill='red')
        # 世
    ...
    bguastr 112212
        ...
        for i in range(6):
            if self.bbgua[i] == '1':
                ...
                self.canvas.create_text(self.guax + 130, 95 + 150 - 30 * i, text=u'▅▅▅', font=('Fixdsys', 17, tkinter.font.BOLD),
                                        tag='pic')
            else:
